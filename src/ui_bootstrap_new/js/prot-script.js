...
xhr.onload = function (e) {
    if (xhr.readyState === 4) {
        if (xhr.status === 200) {
            ...
            dataAllList = JSON.parse(xhr.responseText)
            searchCardMaker(dataAllList)
            dataList = dataAllList.concat()
            console.log(dataList.length)
            console.log(dataAllList.length)
        }
    }
};
window.onload = () => {
...
search.addEventListener("input" , () => {
    ...
    nameCheck(search.value,dataAllList)
    console.log(search.value)
...
/*名前に特定の文字があったら検索結果配列に挿入する*/
function nameCheck(text,list) {
    console.log(text)
    console.log(list.length)
    list.forEach(x => {
        let check = false
        let str = new RegExp(text)
        check = str.test(x.KanaName)
        if(check == true){
            dataList.push(x)
        }
    })
...
function searchAddNumFn(x) {
    if(selectData.includes(dataList[x.dataset.num])) {
        selectItemNum++
    }else {
        selectItemNum--
    }
                    ...
                    `<div class="col-12 col-md-6">`+
                        `<div class="col-12 searchcard d-flex flex-row mx-1">`+
                            ...
                            `<div class="col-8 h-100 mx-3 d-flex align-items-center justify-content-center">`+
                                `<p style="font-weight: 500; opacity: 0.9;" class="nametext text-center m-0" data-num="${num-1}" data-id="${x.Id}">${nameLength(x.Name)}</p>`+
                            `</div>`+
                            `<div class="col-2 d-flex align-items-center justify-content-center">`+
                                `<div class="s_c_check_on" data-id="${x.Id}" data-num="${num-1}">`+
                ...
                let card =
                    `<div class="col-12 col-md-6">`+
                        `<div class="col-12 searchcard d-flex flex-row mx-1">`+
                            ...
                            `</div>`+
                            `<div class="col-8 h-100 mx-3 d-flex align-items-center justify-content-center">`+
                                `<p style="font-weight: 500; opacity: 0.9;" class="text-center nametext m-0" data-num="${num-1}" data-id="${x.Id}">${nameLength(x.Name)}</p>`+
                            `</div>`+
                            `<div class="col-2 d-flex align-items-center justify-content-center">`+
...
function selected(div) {
    ...
    if(selectData.length <= 0) {
        selectData.push(dataList[div.dataset.num])
    }else {
        //console.log(dataList[div.dataset.num] )
        selectData.forEach(x => {
            if(x.Id != div.dataset.num){
                selectData.push(dataAllList[div.dataset.num])
            }else {
                let del = selectData.indexOf(dataAllList[div.dataset.num])
                //console.log(dataList[div.dataset.num])
                //console.log(selectData[del])
                //console.log(del)
                selectData.splice(del, 1)
            }
        })
    }
    ...
    selectData.forEach(x => {
        if(x.Id == div.dataset.id){
            //console.log(dataList)
            //console.log(div.dataset.id)
            selectData.push(dataList[div.dataset.id])
        }else {
            //console.log("b")
            let index = selectData.indexOf(dataList.key);
                        ...
                        <div class="row m-0 table-responsive mt-2 rounded-3" style="height: 142px; width: 95%;">
                            <table class="table bg-light m-0">
                                ...
                                <tbody>
                                    <tr>
                                        <th scope="row" class="clacDataFront">100g</th>
                                        <td class="tableper">${x.Kcalorie}<span>kcal</span></td>
                                        <td class="tableper">${x.protain}<span>g</span></td>
                                        <td class="tableper">${x.fat}<span>g</span></td>
                                        <td class="tableper">${x.carbohydrate}<span>g</span></td>
                                        <td class="tableper">${x.fiber}<span>g</span></td>
                                    ...
                                    <tr class="table-secondary clacData" data-id="${x.Id}">
                                        <th scope="row"><span>${x.inval}</span><span>g</span></th>
                                        <td class="tableval"><span>${clacResult(x.Kcalorie, x.inval)}</span><span>kcal</span></td>
                                        <td class="tableval"><span>${clacResult(x.protain, x.inval)}</span><span>g</span></td>
                                        <td class="tableval"><span>${clacResult(x.fat, x.inval)}</span><span>g</span></td>
                                        <td class="tableval"><span>${clacResult(x.carbohydrate, x.inval)}</span><span>g</span></td>
                                        <td class="tableval"><span>${clacResult(x.fiber, x.inval)}</span><span>g</span></td>
